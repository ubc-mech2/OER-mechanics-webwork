...
filelocation=r'C:\Users\ptemm\Downloads\v5module2\GE 124 Question Database - Module 2 9cc673d992ce47c1b4b615581a5f7d90'
#C:\Users\ptemm\OneDrive\Module 2 Testing'
...
def imgmove(imgline,imgcount,filename,filetowrite,qtype):
  ...
  if imgline in imglinelist:
    ...
    oldname=m.group(1).replace(r'%20',r' ')
    newname=r.group(1).replace(r'%20',r' ')
    #if '%' in imgname:
      ##m = re.search("%/(.*?)\.", imgname)
    ...
    if 'RBSEXX' in imgline:
      newname=filelocation+'\\'+filename+str(imgcount)+'.png'
      newimgname=filename+str(imgcount)+'.png'
    else:
      newimgname=newname+str(imgcount)+'.png'
      newname=filelocation+'\\'+newname+str(imgcount)+'.png'
      
    originpath=filelocation+"\\"+oldname
    ...
    try:
      ...
      fileimgerrors.append(filetowrite)
      filetowrite.write('\n$BR\n$BR\n')
      filetowrite.write(imgline)
      #print(originpath)
      #print(newname)
      pass
...
def translateline(line):
  ...
  line=re.sub('\^\((.*?)\)',r'[`^{\1}`]',line)
  line=line.replace('}^[`^','}[`^').replace(r']}  m[`\overr',r']} m  [`\overr')
  ...
  line=line.replace('{[____]{"$ans0"}  N [`\cdot`] m}','[____]{"$ans0"}  N [`\cdot`] m')
  line=line.replace(r'F_y','[`F_y`]').replace(r'F_x','[`F_x`]').replace(r'F_{y}','[`F_y`]').replace(r'F_{By}','[`F_{By}`]').replace(r'F_{Bx}','[`F_{Bx}`]')
  line=re.sub(r'overrightarrow{(.)}_{`](.*?)}',r'overrightarrow{\1_{\2}}`]',line)
  line=re.sub(r'rightarrow{r_{(.*?)}`]}',r'rightarrow{r_{\1}}`]',line)
  line=line.replace(r'm[`\hat{k}`]','m [`\hat{k}`]').replace(' ft[',' ft [').replace('{3[`\hat{i}`]} m  [`\overrightarrow{F}`]={-5[`\hat{j}`]}  N',r'3[`\hat{i}`] m  [`\overrightarrow{F}`]=-5[`\hat{j}`]  N')
  return line

def mctranslate(data):
  #data=re.search
  data=data.replace(r'\,','').replace('cableA','cable A')
  data=data.replace(r'\,\mathr',' mathr').replace('*Given',' Given')
  ...
  data=data.replace('\;','')
  data=data.replace('\\\\',r'\\')
  data=data.replace('---','').replace('weightDimensions','weight dimensions')
  ...
  data=data.replace('lbCoordinate axes','lb Coordinate axes').replace(r'forceSee','force, See').replace(r'\(\hat{k}\)',r'\(\hat{k}\) ').replace(r'lb\(',r'lb \(')
  data=data.replace(r'\(A_z\)\(k\)(\overrightarrow{B}\)',r'\(A_z\)\(\hat{k}\) \(\overrightarrow{B}\)').replace(r'=\{-1200',r'= -1200').replace(r'1200 N\(\cdot\) m\}',r'1200 N\(\cdot\) m')
  data=data.replace(r'0\(\sum\)',r'0 \(\sum\)')
  ...
  data=data.replace(r'\(\(\overrightarrow{A}_x}\) &',r'\(\overrightarrow{A}_x}\) &').replace(r'\(\(\overrightarrow{A}_x}\)',r'\(\overrightarrow{A}_x\)').replace(r'{A_x\) &',r'\(A_x\) &')
  data=data.replace(r'pin A\(B',r'pin A, \(B').replace(r'pin A\(\overrightarrow{B}\)',r'pin A, \(\overrightarrow{B}\)').replace(r'at pin A\(\overright',r'at pin A, \(\overright').replace(r'point B and C\(\overrig','point B and C, \(\overrig')  
  data=data.replace(r'reaction at point B\(\overr',r'reaction at point B,\(\overr').replace(r'\bold{',r'\bf{')
  data=data.replace(r'}\)\)}',r'}}\)') .replace(r'end{matrix}\)\)',r'end{matrix}\)\)')
  data=data.replace(r'B_\){y',r'\(B_y\)').replace(r'B_\){x',r'\(B_x\)').replace(r'A_\){y',r'A_y\)').replace(r'A_\){x',r'\(A_x\)').replace(r'A_x\)}','A_x\)').replace(r'\(lb}',r' lb ').replace(r'  kN\rightarrow\)',r' \(kN\rightarrow\)').replace(r'\(kN\(\cdot\) m\curvearrow\)',r'\(kN \cdot m\curvearrow\)')
  #data=data.replace(r'Consider the following scenario:','Consider the following scenario:\n$BR\n')
  data=data.replace(r'M_\){A',r'\(M_A\)').replace(r'\(\(A_x\)=',r'\(\(A_x\)=').replace(r'A_y\)=-61.3',r'\(A_y\)=-61.3')
  data=data.replace(r'\(kN\(\cdot\) m\curvearrow\)left',r'\(kN \cdot m\curvearrowleft\)').replace(r'rrightarrow{F_DC}',r'rrightarrow{F_{DC}}').replace(r'\curvearrow\)left',r'\curvearrowleft\)').replace(r'\(kN\(\cdot\) m.',r'\(kN\cdot m\).')
  data=data.replace(r'\(\hat{j}\) \(\overrightarrow{B}\)',r'\(\hat{j}\),  \(\overrightarrow{B}\)')
...
def Multiplechoice():
                  ...
                  if 'Answer' in line and '###' not in line:
                      ...
                      while 'eedback' not in lines[i+num]:
                          if lines[i+num].strip() and '![' not in lines[i+num] and '[]' not in lines[i+num] and '()' not in lines[i+num]:
                              ...
                              while not lines[i+num+count].startswith(nextans) and 'eedback' not in lines[i+num+count] and '---' not in lines[i+num+count]:
  ...
  with open(newfile, 'r') as f:
      ...
      data=''
      for i, line in enumerate(ln):
        if 'image(' not in line and 'width=>' not in line and 'png' not in line:
          line=mctranslate(line)
        data=data+line
        try:
...
for file in glob.glob("*.md"):
    ...
    with open(file, 'r', encoding='utf-8') as f:
        ...
        torf=0
        for i, line in enumerate(lines):
            if 'Question Format' in line or 'STATICS-MPA06' in line:
    ...
    with open(file,'r') as f:
      ...
      for i, line in enumerate(ln): 
        if '![' in line and '.png' in line:
          if line in imglinelist:
            l1=imglinelist.index(line)
            line=line.replace(line,newimglinelist[l1])
            try:
              fileimgerrors.remove(fileimgerrors[i])
            except:
              pass
        data=data+line 
